...
If you are using the [`<PageGraph>`](/starlight-site-graph/components/pagegraph) or [`<PageBacklinks>`](/starlight-site-graph/components/pagebacklinks) components
(which, if you followed the [Getting Started guide](/starlight-site-graph/getting-started), you likely already are),
...
every time you run a development server or build your website, the package will go through all your site's content and
generate a sitemap for you.
...
- Links added by components, Astro loop directives, etc are _not_ included in the sitemap
...
When running in `dev` mode, the sitemap will be stored inside the `config.sitemapConfig.sitemap` object within
