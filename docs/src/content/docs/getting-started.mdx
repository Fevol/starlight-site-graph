...
Check out the [configuration](/starlight-site-graph/configuration) guide to learn more about the plugin's features, or move
on to the installation instructions below to integrate it into your own (Starlight) site.

:::note[Note]
The theme of this website is provided by [Starlight Theme Obsidian](https://fevol.github.io/starlight-theme-obsidian),
which is designed to integrate well with this plugin.
:::
...
<Steps>
    ...
    2. Include the plugin in your Starlight [configuration](https://starlight.astro.build/reference/configuration/#plugins) in the `astro.config.mjs` file.
        ```diff lang="js"
            // astro.config.mjs
	...
	3. Add the Site Graph [content schema](https://docs.astro.build/en/guides/content-collections/#defining-a-collection-schema)
		...
		you will need to define the `src/content/config.ts` file if it doesn't exist yet.
		```diff lang="ts"
...
:::caution[Warning]
If another plugin on your site overrides the default `<PageSidebar>` component
(that is, it adds a custom component to the right sidebar of the site),
it will likely not work, depending on how the plugin is implemented.
Check out this plugin's [`<PageSidebar>`](/starlight-site-graph/components/pagesidebar) component
to see how the [`<PageGraph>`](/starlight-site-graph/components/pagegraph) or
[`<PageBacklinks>`](/starlight-site-graph/components/pagebacklinks) components can be added manually.
:::

## <span style="display: flex; gap: 8px"><Icon name="astro" /> Installation (Astro)</span>

While the plugin is mainly developed with Starlight support in mind,
regular Astro projects can also use this package's features,
though some additional setup is required.
...
<Steps>
	...
	2. Add the sitemap generation integration of the plugin in your `astro.config.mjs` file.
		The `prefetch` option _is_ required and will be enabled automatically if it wasn't set already,
