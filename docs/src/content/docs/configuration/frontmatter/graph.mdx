...
<Tabs>
	<TabItem label="Shown" icon="approve-check">
		The `PageGraph` _will_ be shown on the page.
		```md {4-5}
	...
	<TabItem label="Hidden" icon="close">
		The `PageGraph` _will **not**_ be shown on the page.
		```md {4-5}
		// src/content/docs/my-page.md
		---
		title: My Page
...
## Node Style
The `nodeStyle` frontmatter key allows you to specify the style of the page's corresponding node in the graph.
...
<Tabs>
	...
	<TabItem label='{actions: ["fullscreen"]}' >
		...
		<GraphExample id="base-style-graph-showcase" config={{ graphConfig: { actions: ["fullscreen"] } }} baseConfig={focusSingleNode} sitemap={singleNodeSitemap} slug="node/" />
	</TabItem>
	<TabItem label='{scale: 0.6}' >
		...
		<GraphExample id="base-style-graph-showcase" config={{ graphConfig: { scale: 0.6 } }} baseConfig={focusSingleNode} sitemap={restyleNodes(singleNodeSitemap, new Map([[["**/node/"], {shape: "square", shapeRotation: 45}]]))} slug={"node/"}/>
	</TabItem>
</Tabs>
