...
export function showContextMenu(e: MouseEvent, items: MenuItem[]) {
    ...
    let browser_w = window.innerWidth, browser_h = window.innerHeight;
    let menu_w = menu.offsetWidth, menu_h = menu.offsetHeight;
    if (browser_h - y < menu_h)
        y = y - menu_h;
    if (browser_w - x < menu_w)
        x = x - menu_w;
